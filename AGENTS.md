...
Self-hosted, browser-accessible cloud environment for running Codex in isolated sandboxes.
...
- Security & compliance review (see `.Codex/skills/compliance-boundaries`)
...
- Open-core seam: core exposes APIs; Pro features are separate packages that consume them.
  Never put Tier 1/2-paid logic in core modules. See `.Codex/docs/architecture.md`.
...
- All Codex internals (JSONL parsing, credential paths, usage estimation) go through
  adapter modules in `packages/shared/Codex-adapters/`. One adapter per undocumented surface.

## Workflow

1. `/plan-phase <n>` → you decide scope, `plan-writer` writes `.Codex/docs/phases/phase-<n>.md`
...
5. `/design-handoff <n>` → produce Codex Design prompt for UI phases

## Compliance (non-negotiable)

- Never build features that proxy or broker a user's Codex subscription OAuth through
