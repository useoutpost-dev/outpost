...
- **Credentials** (`apps/server/src/credentials`) — the ONLY module allowed to touch
  Codex auth files. See credential-layer skill.
...
3. All undocumented Codex surfaces (credential paths, JSONL transcripts, usage
   estimation) live in `packages/shared/Codex-adapters/`, one file per surface, so a
   Codex update breaks one adapter, not the app.
