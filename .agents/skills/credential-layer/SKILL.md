...
description: How Codex accounts, subscriptions, and API keys are stored and mounted into sandboxes. Read before touching auth modes, sandbox creation, or account picker.
---

# Credential Layer

## Two layers per sandbox
- **Identity layer** — Codex credentials. Mounted INTO the sandbox at create time.
- **Profile layer** — settings.json, AGENTS.md, MCP config. Per-profile volume.

Never mount the whole `~/.Codex` shared. Mount ONLY the credential file into an
otherwise per-sandbox `~/.Codex` (session data must stay per-sandbox, or usage
...
- Credential paths and file formats via `Codex-adapters/credentials.ts` ONLY.
